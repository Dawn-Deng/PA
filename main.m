%% 主入口：先进行信道建模，再进行初始化建模，再进行信号建模，最后进行优化问题建模
clear; clc;

[channelState, params] = miso_channel_model();
[initState, params] = miso_initialization(channelState, params);
...
disp('每个用户的峰值潜力 E_max：');
disp(initState.Emax);

disp('候选用户集合 C：');
disp(initState.candidatePool);

disp('参考位置向量 y_ref：');
disp(initState.referencePositions);

disp('匹配效用矩阵 U：');
disp(initState.utilityMatrix);

disp('初始服务集合 S^(0)：');
disp(initState.initialServiceSet);

disp('名义初始位置矩阵 Xbar^(0)：');
disp(initState.Xbar0);

disp('投影后的初始位置矩阵 X^(0)：');
disp(initState.X0);

disp('初始俯仰角 theta^(0)：');
disp(initState.theta0);

disp('初始方位角 phi^(0)：');
disp(initState.phi0);

disp('初始匹配结果：');
disp(struct2table(initState.matching.selectedPairs));
