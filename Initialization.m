...
% 工程化增强：用于批量实验统计与论文出图的数据摘要
    ...
    diagnostics.maxGpot = max(Gpot(:));
    diagnostics.meanGpot = mean(Gpot(:));
    diagnostics.meanMovementCost = mean(moveFlat);
    diagnostics.lambdaMovTimesMeanMovementCost = lambdaMov * mean(moveFlat);
